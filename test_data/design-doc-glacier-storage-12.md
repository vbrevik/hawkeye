...
We will add structured logging with trace IDs using GraphQL as the primary technology.

### Architecture

```
[Client] → [auth-service] → [data-warehouse] → [Database]
                ↓
          [GraphQL]
```
...
2. auth-service validates the token using Kafka
