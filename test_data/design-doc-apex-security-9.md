...
We will add structured logging with trace IDs using SQLite as the primary technology.

### Architecture

```
[Client] → [audit-logger] → [media-uploader] → [Database]
                ↓
          [SQLite]
```
...
2. audit-logger validates the token using RabbitMQ
