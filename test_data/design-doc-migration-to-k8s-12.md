...
This document describes the design for Migration to K8s. The goal is to implement circuit breakers for downstream calls
...
We will document the deployment process using Prometheus as the primary technology.

### Architecture

```
[Client] → [auth-service] → [scheduler] → [Database]
                ↓
          [Prometheus]
```
...
2. auth-service validates the token using Helm
