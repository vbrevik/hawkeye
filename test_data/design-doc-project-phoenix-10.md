...
This document describes the design for Project Phoenix. The goal is to add structured logging with trace IDs
in a way that is scalable, maintainable, and observable.

## Problem Statement

Currently, media-uploader suffers from token expiry edge case when clock skew > 30s.
This leads to degraded performance and reliability. We need a solution
that handles the current load and scales to 10x.

## Proposed Solution

We will migrate the legacy monolith to microservices using Grafana as the primary technology.

### Architecture

```
[Client] → [media-uploader] → [user-service] → [Database]
                ↓
          [Grafana]
```
...
2. media-uploader validates the token using Helm
