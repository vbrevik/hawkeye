...
This document describes the design for Nexus API. The goal is to review and rotate all secrets in Vault
...
We will implement circuit breakers for downstream calls using Rust as the primary technology.

### Architecture

```
[Client] → [api-gateway] → [analytics-pipeline] → [Database]
                ↓
          [Rust]
```
...
2. api-gateway validates the token using PostgreSQL
